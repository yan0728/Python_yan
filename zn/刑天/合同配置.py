...
'''
# 签署任务的uuid
missionuuid = 'e0f0948d-09ab-4058-8431-9826973d52cf'
getdata2 = None
getdata3 = None
getdata4 = None
getdata5 = None
...
def setContract7():
    ...
    if r.json()["code"] == "SUCCESS":
        ...
        print("7-《应收账款转让通知书》适用于原始债权人向共同债务人出具 配置失败")
        print(json.dumps(r.json(), sort_keys=True, indent=2, ensure_ascii=False))
# 上传合同文件
def uploadContractFile7():
    ...
    r = requests.post(url=url, data=json.dumps(parameter), headers=header)
    if r.json()["code"] == "SUCCESS":
        print("7-《应收账款转让通知书》适用于原始债权人向共同债务人出具 配置并上传完成")

    else:
        print("7-《应收账款转让通知书》适用于原始债权人向共同债务人出具 上传失败")
        print(json.dumps(r.json(), sort_keys=True, indent=2, ensure_ascii=False))
...
def uploadContractFile14():
    ...
    if r.json()["code"] == "SUCCESS":
        ...
        print("114-《付款确认文件》适用于共同债务人向原始权益人出具 上传失败")
        print(json.dumps(r.json(), sort_keys=True, indent=2, ensure_ascii=False))
